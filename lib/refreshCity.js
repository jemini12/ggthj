...
async function refreshCityFull({ city, fromYmd, toYmd: toYmdStr, timeoutMs = 10000 }) {
  ...
  async function worker() {
    while (idx < days.length) {
      ...
      const statusCounts = result && result.statusCounts ? result.statusCounts : null;
      const date = toYmd(day);
      points[myIdx] = { date, count, status: statusCounts };
      log("refreshCity.day", { date, count, status: statusCounts });
    }
  }
  const workers = [];
