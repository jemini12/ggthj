...
export default function StackedChart({ series, width = 1000, height = 380, pad = 52 }) {
  ...
  }, [series, effectiveWidth, height, pad, topPad, bottomPad, leftPad, rightPad]);

  const hoverBar = hover != null ? bars[hover] : null;

  return (
    <div className="w-full" onClick={() => setHover(null)}>
        ...
        >
          ...
          {hoverBar ? (
            <g pointerEvents="none">
              <rect
                x={Math.min(effectiveWidth - 230, Math.max(leftPad, hoverBar.x - 110))}
                y={pad + 6}
                width="220"
                height="74"
                rx="12"
              ...
              />
              <text
                x={Math.min(effectiveWidth - 220, Math.max(leftPad + 12, hoverBar.x - 98))}
                y={pad + 28}
                fontSize="12"
                fill="#e5e7eb"
              >
